...
public class CommunityDAO extends DBConnPool
{
	...
	/*
	  	...
	  	모델2 방식에서는 community테이블 및 communityDTO를 사용하므로
	  	해당 코드만 수정하면 된다.
	 */
	...
	public CommunityDTO selectView(String idx)
	{
		...
		String query = "SELECT * FROM community WHERE idx=?";
	...
	public boolean confirmIdx(String idx)
	{
		...
		try
		{
			String sql = "SELECT COUNT(*) FROM community WHERE idx=?";
			psmt = con.prepareStatement(sql);
			psmt.setString(1, idx);
			rs = psmt.executeQuery();
	...
	public int deletePost(String idx)
	{
		...
		try
		{
			String query = "DELETE FROM community WHERE idx=?";
			psmt = con.prepareStatement(query);
			psmt.setString(1, idx);
			result = psmt.executeUpdate();
